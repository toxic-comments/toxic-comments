...
В ходе проведённого анализа также были выявлены недостатки данного подхода. Основными из них являются большой размер матрицы,
используемой для построения таких эмбеддингов, а также нестабильность её факторизации методом SVD. В следующем чекпоинте мы
попробуем обучить эмбеддинги Word2Vec, которые по своей сути являются аппроксимацией count-based подхода.