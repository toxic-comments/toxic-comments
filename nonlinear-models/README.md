# Нелинейные ML-модели и кластеризация
...
| tf-idf + svd          | CatBoost           | 0.67     |
| BoW                   | RandomForest       | 0.65     |
| w2v                   | LightGBM           | 0.64     |
| w2v + custom features | CatBoost + optuna  | 0.64     |
...
### Модель CatBoost. w2v araneum + custom features

Аналогично предыдущей модели обучили также catboost и подобрали гиперпараметры. Качество после подбора гиперпараметров **F1 macro = 0.64** получилось заментно ниже, чем у lightGBM.

### Модель CatBoost. признаки - TF-iDF

Обучили catboost на разряженной матрице TF-iDF. Всего 17 000 признаков - 12 тысяч n-gram для токенов + 5 тысяч посимвольных n-gram. Получилось добиться качества **F1 macro = 0.74**. 

Также попробовали понизить размерность матрицы признаков с помощью SVD. Качество упало до 0.67. 

Выводы по катбусту - он лучше сработал с Count-based representations признаками, а не эмбеддингами. То же самое мы наблюдали и для линейных моделей. 
...
В ходе работы над этим чекпоинтом была выполнена кластеризация комментариев. Для этого были проделаны следующие шаги:

1. Для всего корпуса комментариев была построена матрица совстречаемости слов в контекстном окне размером 5 (включая центральное слово)
2. Для данной матрицы было выполнено PCA разложение, позволившее выделить скрытые направления наибольшей вариативности,
   позволившие снизить размерность исходной матрицы до vocab_size x 250 (подробнее про размерность словаря и эмбеддинга
   [тут](clusterisation.ipynb)).
    
   Визуализация семантического пространства слов:  <a href="https://toxic-comments.github.io/toxic-comments/nonlinear-models/reports/reports/words_semantic_space.html" target="_blank">тык</a>
3. Каждый комментарий представлен как среднее из эмбеддигов слов в него входящих, визуализации <a href="https://toxic-comments.github.io/toxic-comments/nonlinear-models/reports/reports/comments_semantic_space.html" target="_blank">тут</a> 
4. На получившихся эмбеддингах была проведена кластеризация (<a href="https://toxic-comments.github.io/toxic-comments/nonlinear-models/reports/comments_semantic_space.html" target="_blank">визуализация</a>). 
   Normalized mutual information между классом токсичности комментария и выделенным кластером для лучшей модели равна 0.13, 
   что не очень много, но все равно может быть использовано в качестве эстиматора для класса токсичности.

В ходе проделанного анализа так же были найдены недостатки такого подхода, которые заключаются в основном в большом размере
матрицы для получения такого рода эмбеддингов, а так же в нестабильности факторизации такой матрицы через SVD.
В следующем чекпоинте мы попробуем подойти к задаче обучения эмбедингов типа Word2Vec, который в сущности апроксимирует
count based подход.