# Описание седьмого чекпоинта

## Выбор модели

В качестве итоговой модели была выбрана архитектура BERT на основе предобученной модели
DeepPavlov/rubert-base-cased-conversational. Данная модель была предварительно обучена на задачах MLM и NSP с 
использованием корпусов неформального русскоязычного текста, собранных из форумов и социальных сетей. На следующем этапе
модель была дообучена на целевую задачу классификации.

| Название модели                             | F1-macro | Время инференса (ms) | Занимаемое место (MB) |
|---------------------------------------------|----------|----------------------|-----------------------|
| DeepPavlov/rubert-base-cased-conversational | **0.90** | **214.987**          | 14.290                |
| cointegrated/rubert-tiny2                   | 0.85     | 12.067               | 10.097                |
| LSTM                                        | 0.83     | 54.371               | 0.040                 |
| Logistic Regression + BoW (baseline)        | 0.82     | 24.055               | **46.954**            |

Несмотря на сравнительно большое время инференса, что не является существенным ограничением для бота, предназначенного
для маркировки неуместных сообщений в чатах, данная архитектура демонстрирует лучшее качество классификации с точки
зрения баланса между recall и accuracy.

## Перенос экспериментов в mlflow

Все финальные эксперименты проводились в среде версионирования экспериментов MLflow, развернутой на сервере в связке
с объектным хранилищем S3. Дополнительно в репозитории был настроен CD-пайплайн, обеспечивающий непрерывную доставку
изменений конфигурации на сервер деплоймента.

Настроенный mlflow доступен по адресу: http://70.34.242.179/#/experiments

Код всех экспериментов с финальными архитектурами представлен в ноутбуке [experiments.ipynb](experiments.ipynb), для
которого настроено подключение к среде экспериментов. Для каждой архитектуры в MLflow создан отдельный эксперимент,
включающий несколько запусков с различными значениями гиперпараметров. Значения всех гиперпараметров и seed сохранены в
параметрах соответствующих запусков. Финальные версии моделей, продемонстрировавшие наилучшее качество, сохранены в
объектном хранилище с тегом `final`. 

Ноутбук с загрузкой финальных версий моделей: [models_loading_demo.ipynb](models_loading_demo.ipynb)
